...
describe('K8S operator', () => {
    ...
    // it('Get the selectors of mysql K8S service', async () => {
    //     // const status = await k8sApiPods.listNamespacedService(Config.WATCH_NAMESPACE);
    //     const status = await k8sApiPods.readNamespacedService(Config.MYSQL_SERVICE_NAME, Config.WATCH_NAMESPACE);
    //     console.log(status.response.body.spec.selector['app.kubernetes.io/name']);
    // });
    ...
    // TODO: The following is not working for interpod communication, scale down the mysql stateful-set to disable it
    // it('Add a selector to mysql K8S service to disable it', async () => {
    //     const patch = [
    //         {
    //             op: 'add',
    //             path: '/spec/selector/app.kubernetes.io~1disable',
    //             value: 'disable',
    //         },
    //     ];
    //     const options = { headers: { 'Content-type': k8s.PatchUtils.PATCH_FORMAT_JSON_PATCH } };
    //     const status = await k8sApiPods.patchNamespacedService(
    ...
    it('Get the status of the custom resource', async () => {
        ...
        expect(status.response.body).toHaveProperty('status');
        expect(status.response.body.status).toHaveProperty('state');
        expect(status.response.body.status.state).toEqual('INVALID');
    ...
    // TODO: The following is not working for interpod communication, scale up the mysql stateful-set to re-enable it
    // it('Remove the disabled selector to re-enable mysql K8S service', async () => {
    //     const patch = [
    //         {
    //             op: 'remove',
    //             path: '/spec/selector/app.kubernetes.io~1disable',
